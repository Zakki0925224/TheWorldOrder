...
namespace ShpLoader
{
    public class ShpFile : IShpFile, IRenderable
    {
        ...
        public void Render(Color color, DbfFile dbfFile, GameObject parentGameObject)
        {
            for (int i = 0; i < RecordSet.Count; i++)
            {
                ...
                for (int j = 0; j < dbfFile.FieldList.Count; j++)
                {
                    ...
                    var data = dbfRecord.Record[j].ToString().Trim();

                    record.Add(disc.FieldName, data);
                    recordList.Add(data);
                }
    ...
    public class ShxFile : IShpFile, IDisposable, IRenderable
    {
        private bool disposed;
        private FileStream fs;
        private BinaryReader br;
        ...
        public void Render(Color color, DbfFile dbfFile, GameObject parentGameObject)
        {
            ContentsFile.Render(color, dbfFile, parentGameObject);
        }
